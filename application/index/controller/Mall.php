...
class Mall
{
    public function mallIndex()
    {
        ...
        if ($goods_class == 1) {
            //商品列表
            $selectgoods = Db::table('ml_tbl_goods')->where('is_online', 1)->limit($start, $limit)->select();
            if ($selectgoods) {
            ...
            $selectgoods = Db::table('ml_tbl_goods')->where('is_online', 1)->where('goods_class',$goods_class)->limit($start, $limit)->select();
            if ($selectgoods) {
                $data = array('status' => 0, 'msg' => '成功', 'data' => $selectgoods);
                return json($data);
            } else {
    ...
    public function goodsDetails()
    {
        ...
        $selectgoodsbanner = Db::table('ml_tbl_goods_banner')->where('goods_id',$goods_id)->select();
        if ($selectgoods) {
